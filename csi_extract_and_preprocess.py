...
def load_exam_window_ns(json_path: Path, duration_s: int) -> Tuple[int, int]:
    """Read ts_start and ts_end (nanoseconds) from the sidecar JSON file."""
    with open(json_path) as f:
        data = json.load(f)
    responses = data.get("responses", [])
    if not responses:
        meta_data = data.get("examMetadata", [])
        if not meta_data:
            raise ValueError(f"No timestamp data found in {json_path}!")
        start_time = str(meta_data["startTime"])
        start_ns = datetime_to_ts(start_time)
...
def preprocess_csi(ts_arr, csi_arr, args):
    ...
    PLACEHOLDER: Interpolate the CSI matrix to a constant frequency
    ...
    """
    # -----------------------------------------------------------------------
    # Interpolate the CSI matrix to a constant frequency
    # -----------------------------------------------------------------------
    start_ts = ts_arr[0]
    end_ts = ts_arr[-1]
    test_duration = args.test_duration
    freq = args.interpolate_freq
...
def process_subdir(subdir: Path, output_dir: Path, args) -> None:
    ...
    try:
        ts_start, ts_end = load_exam_window_ns(json_path, args.test_duration)
...
def parse_args():
    ...
    p.add_argument("--interpolate_freq", type=int, default=500)
